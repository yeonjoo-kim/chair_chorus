...
const state = {
  ...
  micStatus: "unknown",
  micLevel: 0,
  timerTotal: FAST_MODE ? 8 : 300,
  timerLeft: FAST_MODE ? 8 : 300,
  timerRunning: false,
  timerId: null,
  submitted: false,
  sliders: {
...
function renderMake2(showModal) {
  const screen = screenShell();
  screen.innerHTML = timerMarkup("Your portrait<br>will be completed in 5 minutes.") + controlsMarkup() + chairMarkup({ color: currentNoiseColor(), intensity: 0, alignMake: true });
  bindControls(screen);
...
function renderMake3(locked) {
  ...
  const message = state.timerLeft > 210 ? "Please wait a minute..." : state.timerLeft > 90 ? "The portrait is listening..." : "Almost grown.";
...
function renderEnding() {
  ...
  screen.innerHTML = html`
    <div class="photo-zone">
      <div class="ending-paints">${endingPaintMarksMarkup()}</div>
      ...
      <div class="ending-actions">
        ...
        <a class="button green download-character" data-action="download" href="#" download="chair-character.svg">Download the chair</a>
      </div>
  ...
  const downloadLink = screen.querySelector("[data-action='download']");
  prepareCharacterSvgDownloadLink(downloadLink);
  prepareInlinedCharacterSvgDownloadLink(downloadLink);
  downloadLink.addEventListener("click", event => {
    event.preventDefault();
    showChairSnapshotOverlay(screen);
  });
}

function endingPaintMarksMarkup() {
  const slots = [
    [120, 330], [1680, 350],
    [260, 510], [1540, 540],
    [410, 690], [1390, 700]
  ];
  const marks = slots.map((_, index) => ({
    src: PAINTS[index % PAINTS.length],
    width: 150,
    rotation: [-16, 18, -24, 26][index % 4]
  }));
  return slots.map(([x, y], index) => {
    const mark = marks[index];
    const width = index % 2 === 0 ? 92 : 104;
    const rotation = Number.isFinite(mark.rotation) ? mark.rotation : [-18, 20, -26, 24][index % 4];
    const src = PAINTS[Math.floor(index / 2) % PAINTS.length];
    return `<img class="ending-paint" src="./assets/${src}" alt="" style="left:${x}px;top:${y}px;width:${width}px;--paint-r:${rotation}deg">`;
  }).join("");
}

async function showChairSnapshotOverlay(screen) {
  const character = state.character || getChorusCharacters()[0] || {};
  const rawSvg = verticalCharacterSvgString(character);
  if (!rawSvg) return;
  let snapshotSvg = rawSvg;
  try {
    snapshotSvg = await inlineSvgImageAssets(rawSvg);
  } catch (error) {
    console.warn("Could not inline snapshot assets.", error);
  }
  const imageUrl = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(snapshotSvg)}`;
  const filename = `${(character.nickname || "chair-character").replace(/\s+/g, "-")}.svg`;
  screen.querySelector(".snapshot-overlay")?.remove();
  const overlay = document.createElement("div");
  overlay.className = "snapshot-overlay";
  overlay.innerHTML = html`
    <div class="snapshot-frame">
      <img class="snapshot-image" src="${imageUrl}" alt="chair snapshot">
      <div class="snapshot-actions">
        <button class="button green snapshot-save" type="button">Preparing...</button>
        <button class="button snapshot-close" type="button">Close</button>
      </div>
    </div>
  `;
  screen.appendChild(overlay);
  const saveButton = overlay.querySelector(".snapshot-save");
  const pngName = filename.replace(/\.svg$/, ".png");
  let preparedBlob = null;
  prepareSnapshotPngBlob(snapshotSvg).then(blob => {
    preparedBlob = blob;
    saveButton.textContent = blob ? "Save image" : "Save failed";
  });
  saveButton.addEventListener("click", async () => {
    if (!preparedBlob) {
      saveButton.textContent = "Preparing...";
      preparedBlob = await prepareSnapshotPngBlob(snapshotSvg);
    }
    if (preparedBlob) await saveBlobAsFile(preparedBlob, pngName, saveButton);
  });
  overlay.querySelector(".snapshot-close").addEventListener("click", () => overlay.remove());
}
...
function sliderMarkup(locked) {
  ...
  return html`
    <div class="slider-panel">
      ...
      <div class="slider-title">How tired are you from these?</div>
      ${rows}
    </div>
...
function downloadCharacterSvgString(character = state.character || getChorusCharacters()[0]) {
  if (!character) return;
  return chairMarkup({
    color: character.color,
    intensity: character.intensity,
    hairLevel: character.hairLevel,
    beardLevel: character.beardLevel
  }).match(/<svg[\s\S]*<\/svg>/)[0];
}

function verticalCharacterSvgString(character = state.character || getChorusCharacters()[0]) {
  const rawSvg = downloadCharacterSvgString(character);
  if (!rawSvg) return;
  const hair = Math.min(6, Math.max(1, Math.round(character.hairLevel || levelFromIntensity(character.intensity || 0))));
  const beard = Math.min(6, Math.max(1, Math.round(character.beardLevel || levelFromIntensity(character.intensity || 0))));
  const hairBox = HAIR_LAYOUTS[hair];
  const beardBox = BEARD_LAYOUTS[beard];
  const minX = Math.min(20, hairBox.x, beardBox.x);
  const maxX = Math.max(558, hairBox.x + hairBox.w, beardBox.x + beardBox.w);
  const minY = Math.min(20, hairBox.y, beardBox.y);
  const maxY = Math.max(854, hairBox.y + hairBox.h, beardBox.y + beardBox.h);
  const contentW = maxX - minX;
  const contentH = maxY - minY;
  const scale = Math.min(.62, 700 / contentW, 1040 / contentH);
  const tx = 450 - ((minX + maxX) / 2) * scale;
  const ty = 800 - ((minY + maxY) / 2) * scale;
  const innerSvg = rawSvg
    .replace(/<svg class="chair-svg[^>]*>/, "")
    .replace(/<\/svg>\s*$/, "");
  return html`
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 1600" width="900" height="1600">
      <rect width="900" height="1600" fill="#fff"/>
...
async function inlineSvgImageAssets(svg) {
  const matches = [...svg.matchAll(/href="([^"]+\.svg)"/g)];
  let nextSvg = svg;
  for (const [, href] of matches) {
    const response = await fetch(href);
    if (!response.ok) throw new Error(`Could not load SVG asset: ${href}`);
    const assetText = await response.text();
    const dataUrl = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(assetText)}`;
    nextSvg = nextSvg.split(href).join(dataUrl);
  }
  return nextSvg;
}

async function createCharacterPngUrl() {
  const rawSvg = verticalCharacterSvgString();
  if (!rawSvg) return null;
  let svg;
  try {
    svg = await inlineSvgImageAssets(rawSvg);
  } catch (error) {
    console.warn("Could not inline SVG assets for download.", error);
    svg = rawSvg;
  }
  if (!svg) return null;
    ...
    image.onload = () => {
      ...
      try {
        canvas.toBlob(pngBlob => {
          ...
          window.__lastCharacterDownload = { size: pngBlob.size };
          resolve(pngUrl);
        }, "image/png");
      } catch (error) {
        console.warn("Could not export character PNG.", error);
        resolve(null);
      }
    };
    image.src = url;
...
async function prepareSnapshotPngBlob(svg) {
  ...
  return new Promise(resolve => {
    ...
    image.onerror = () => {
      console.warn("Could not render snapshot for download.");
      resolve(null);
    };
    image.src = imageUrl;
  });
...
async function saveBlobAsFile(blob, filename, button) {
  ...
  try {
    if (window.showSaveFilePicker) {
      ...
      window.setTimeout(() => { button.textContent = "Save image"; }, 1200);
      return;
    }
  } catch (error) {
    ...
    console.warn("File picker failed; falling back to download link.", error);
  }
  const url = URL.createObjectURL(blob);
...
async function createCharacterSvgDownloadUrl() {
  const rawSvg = verticalCharacterSvgString();
  if (!rawSvg) return null;
  let svg = rawSvg;
  try {
    ...
    console.warn("Could not inline SVG assets for SVG download.", error);
  }
  const blob = new Blob([svg], { type: "image/svg+xml;charset=utf-8" });
  return URL.createObjectURL(blob);
}

function prepareCharacterSvgDownloadLink(link) {
  const character = state.character || getChorusCharacters()[0] || {};
  const rawSvg = verticalCharacterSvgString(character);
  if (!rawSvg) return;
  link.href = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(rawSvg)}`;
  link.download = `${(character.nickname || "chair-character").replace(/\s+/g, "-")}.svg`;
  link.textContent = "Download the chair";
  link.classList.remove("disabled");
  link.removeAttribute("aria-disabled");
}

async function prepareInlinedCharacterSvgDownloadLink(link) {
  const character = state.character || getChorusCharacters()[0] || {};
  const rawSvg = verticalCharacterSvgString(character);
  if (!rawSvg) return;
  try {
    const svg = await inlineSvgImageAssets(rawSvg);
    link.href = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`;
    link.download = `${(character.nickname || "chair-character").replace(/\s+/g, "-")}.svg`;
  } catch (error) {
    console.warn("Could not prepare inlined SVG download; using basic SVG link.", error);
  }
}

async function createCharacterDownloadUrl() {
  const pngUrl = await Promise.race([
    createCharacterPngUrl(),
    new Promise(resolve => setTimeout(() => resolve(null), 1800))
  ]);
  if (pngUrl) return { url: pngUrl, extension: "png" };
  const svgUrl = await createCharacterSvgDownloadUrl();
  if (svgUrl) return { url: svgUrl, extension: "svg" };
  return null;
}

async function prepareCharacterDownloadLink(link) {
  link.textContent = "Preparing...";
  link.classList.add("disabled");
  link.setAttribute("aria-disabled", "true");
  const download = await createCharacterDownloadUrl();
  if (!download) {
  ...
  link.dataset.objectUrl = download.url;
  link.download = `${(character.nickname || "chair-character").replace(/\s+/g, "-")}.${download.extension}`;
  link.textContent = "Download the chair";
  link.classList.remove("disabled");
  link.removeAttribute("aria-disabled");
  window.__lastCharacterDownload = { ...(window.__lastCharacterDownload || {}), filename: link.download };
}

async function downloadCharacterPng() {
  const link = document.createElement("a");
  await prepareCharacterDownloadLink(link);
  if (!link.href) return;
  document.body.appendChild(link);
  link.click();
  link.remove();
