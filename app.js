...
function renderEnding() {
  ...
  screen.innerHTML = html`
    <div class="photo-zone">
      ...
      <div class="ending-actions">
        ...
        <a class="button green download-character" data-action="download" href="#" download="chair-character.png">Download Character</a>
      </div>
  ...
  downloadLink.addEventListener("click", event => {
    if (downloadLink.getAttribute("href") === "#") {
      ...
      prepareCharacterDownloadLink(downloadLink);
    }
  });
  prepareCharacterDownloadLink(downloadLink);
}
...
async function createCharacterPngUrl() {
  const rawSvg = downloadCharacterSvgString();
...
async function prepareCharacterDownloadLink(link) {
  const pngUrl = await createCharacterPngUrl();
  if (!pngUrl) return;
  const character = state.character || getChorusCharacters()[0] || {};
  if (link.dataset.objectUrl) URL.revokeObjectURL(link.dataset.objectUrl);
  link.href = pngUrl;
  link.dataset.objectUrl = pngUrl;
  link.download = `${(character.nickname || "chair-character").replace(/\s+/g, "-")}.png`;
